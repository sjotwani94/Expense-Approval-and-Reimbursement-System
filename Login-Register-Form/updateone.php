	...
	if (isset($_POST['submit']))
	{
		if($Action=='Update')
		{
			...
			else
			{
				...
				$query="update registration set Name='$Name',Department='$Department',Designation='$Desgn',Email_ID='$Email',Password='$Password',Confirm_Pass='$ConfirmPassword' where Email_ID='$EmailID'";
		...
		else if($Action=='newdata')
		{
			...
			else
			{
				...
				{
				// SQL query to fetch information of registerd 	users and finds user match.
				$query="insert into registration(Name,Department,Designation,Email_ID,Password,Confirm_Pass) values('$Name','$Department','$Desgn','$Email','$Password','$ConfirmPassword')";
				$result = mysqli_query($conn,$query);
				$rows = mysqli_num_rows($result);
				?>
				<script type="text/javascript">
